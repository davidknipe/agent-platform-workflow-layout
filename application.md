...
The layout engine lives in `opal-workflow-layout.ts` — the single source of truth. It compiles to `opal-workflow-layout.js` (ES module), which is loaded by the browser tool and can be imported by other TypeScript/JavaScript applications.

## Files

- `opal-workflow-layout.ts` — **single source of truth** for all layout logic
- `opal-workflow-layout.js` — compiled output (committed; served by Cloudflare alongside `index.html`)
- `opal-workflow-layout.d.ts` — TypeScript declarations for consumers (generated by `tsc`)
- `index.html` — browser tool: UI, styles, and a `<script type="module">` that imports from `opal-workflow-layout.js`
...
3. Commit **both** `opal-workflow-layout.ts` and `opal-workflow-layout.js` (the compiled file is served as a static asset by Cloudflare Pages — there is no build step on deploy)
4. Push to `main` — Cloudflare deploys automatically

### Making UI-only changes
Edit `index.html` directly. No rebuild needed unless engine logic also changed.

### Browser tool structure (`index.html`)
- `<script src="./opal-workflow-layout.js">` — loads the compiled engine
- `<script type="module">` — UI IIFE: imports `{ computeLayout, buildOutput, layoutStats }` from the engine, then handles paste/copy, controls, SVG preview rendering, stats chips, toasts
- No inline engine logic — the UI script is ~300 lines and contains no layout algorithm code
...
- Keep the CSP meta tag (`default-src 'none'; script-src 'self' 'unsafe-inline'; style-src 'unsafe-inline'; img-src data:`). `'self'` is required to load `opal-workflow-layout.js` from the same origin. No other network calls, no external resources, no eval.
